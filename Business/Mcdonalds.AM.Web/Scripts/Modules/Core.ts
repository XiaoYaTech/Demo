...
angular.module("mcd.am.modules", [
    ...
    "dictionaryServices"
]).directive("taskReminder", ["$http", ($http: ng.IHttpService): ng.IDirective=> {
        return {
            restrict: "EA",
            replace: true,
                    ...
                    $scope.$watch("refresh", (val) => {
                        ...
                        if (!!val && val === true) {
                            ...
                            $http.get(url).success((data: any) => {
                                angular.forEach(data.Nodes, (checkPoint, i) => {
    ...
    .directive("step1", [
        ...
        "messager",
        "storeService",
        ($http: ng.IHttpService, $modal: ng.ui.bootstrap.IModalService, messager, storeService): ng.IDirective=> {
            return {
                restrict: "EA",
                replace: true,
                scope: {
                ...
                link: ($scope: any, element: JQuery, attrs: any) => {
                    ...
                    $scope.selectStore = () => {
                        $modal.open({
                            ...
                            controller: ["$scope", "$modalInstance", "condition", ($modalScope: any, $modalInstance: ng.ui.bootstrap.IModalServiceInstance, condition) => {
                                ...
                                $modalScope.list = [];
                                $modalScope.$watch("pageIndex", (page: number) => {
                                    $modalScope.pagging();
                                });
                                ...
                                $modalScope.ok = (store: any) => {
                                    $modalInstance.close(store);
                                };

                                $modalScope.search();
                            }]
                        }).result.then((store: any) => {
    ...
    .directive("storeBasicInfo", [
        ...
        "projectUsersService",
        "$routeParams",
        ($http: ng.IHttpService, projectUsersService, $routeParams): ng.IDirective=> {
            return {
                restrict: "EA",
                scope: {
                    code: "=",
                ...
                link: ($scope: any, ele: JQuery, attrs: any) => {
                    ...
                    $scope.$watch("code", (val) => {
                        if (!!val && val.length == 7 && !isNaN(val)) {
                            $http.get(Utils.ServiceURI.Address() + "api/Store/Details/" + val).success((data) => {
                                $scope.store = data;
    ...
    .directive("projectTeam", [
        ...
        "projectUsersService",
        "projectService",
        (messager, projectUsersService, projectService): ng.IDirective=> {
            return {
                restrict: "EA",
                scope: {
                    projectId: "@",
                ...
                link: ($scope, element: JQuery, attrs: any) => {
                    $scope.team = {};
                    $scope.$watch("usCode", (val) => {
                        if (!!val && val.length == 7 && !isNaN(val)) {
                            if ($scope.projectId) {
                                projectUsersService.canInnerEdit($scope.projectId).then((response: any) => {
                                    $scope.canInnerEdit = response.data;
                                });
                            }
                            projectUsersService.queryTeamUser($scope.projectId, $scope.usCode)
                                .then((response: any) => {
    ...
    .directive('notifyUsersModal', [
        ...
        ($modal: ng.ui.bootstrap.IModalService, projectUsersService, messager): ng.IDirective => {
            return {
                restrict: "EA",
                scope: {
                    editable: '=',
                    ...
                    $scope.$watch('visible', (val) => {
                        if (val) {
                            ...
                            $modal.open({
                                ...
                                size: 'lg',
                                resolve: {
                                    team: () => angular.copy($scope.team),
                                    notifyUsersInfo: () => angular.copy($scope.notifyUsersInfo)
                                },
                                controller: [
                                    "$scope", "$modalInstance", "$selectUser", "team", "notifyUsersInfo", (noticeScope, $modalInstance, $selectUser, team, notifyUsersInfo) => {
                                        ...
                                        noticeScope.selectEmployee = () => {
                                            var users = noticeScope.notifyUsersInfo.NoticeUsers;
                                            if (users) {
                                                users = $.map(noticeScope.notifyUsersInfo.NoticeUsers, (u, i) => {
                                                    return {
                                                        Code: u.UserAccount,
                                            ...
                                            $selectUser.open({
                                                ...
                                                OnUserSelected: (selectedUsers: any[]) => {
                                                    noticeScope.noticeUsersName = $.map(selectedUsers, (u, i) => {
                                                        return u.NameENUS;
                                                    }).join(",");
                                                    noticeScope.notifyUsersInfo.NoticeUsers = $.map(selectedUsers, (u, i) => {
                                                        return {
                                                            UserAccount: u.Code,
    ...
    .directive("contractInfo", [
        ...
        "$http",
        (contractService, contractRevisionService, $http): ng.IDirective=> {
            return {
                restrict: "EA",
                scope: {
                    editable: "=",
                ...
                link: ($scope: any, element: JQuery, attrs: any) => {
                    ...
                    var loadRevisions = (contractId: string) => {
                    ...
                    var parseYear = (dateStr: string, prop: string) => {
                        if (!!dateStr) {
                    ...
                    $scope.title = "";
                    contractService.query($scope.projectId).then((response: any) => {
                    ...
                    $scope.removeRevision = (revision) => {
                        $scope.revisions = $.grep($scope.revisions, (re, i) => {
                            return re !== revision;
                        });
                    };
                }
    ...
    .directive("comment", [(): ng.IDirective=> {
        return {
            restrict: "EA",
            scope: {
                editable: "=",
    ...
    }]).directive("recall", ["$modal",
        ($modal: ng.ui.bootstrap.IModalService): ng.IDirective => {
            return {
                restrict: "E",
                scope: {
                    callBack: "&"
                ...
                link: ($scope: any, element: JQuery, attrs: any) => {
                    ...
                    $scope.beginReCall = () => {
                        ...
                        $modal.open({
                            ...
                            controller: [
                                "$scope", "$modalInstance", "condition", ($scope: any, $modalInstance: ng.ui.bootstrap.IModalServiceInstance, condition) => {
    ...
    .directive('attachments', [
        ...
        "contractService",
        "messager",
        ($modal, $document, attachmentService, contractService, messager): ng.IDirective=> {
            return {
                restrict: "E",
                scope: {
                    refTableName: "@",
                ...
                link: ($scope: any, element: JQuery, attrs: any) => {
                    ...
                    /*事件订阅 End*/
                    $scope.uploadAttachFinished = (up, files) => {
                            ...
                            controller: [
                                ...
                                ($modalScope, $modalInstance: ng.ui.bootstrap.IModalServiceInstance) => {
                                    contractService.queryAttachments($scope.projectId).then(function (response) {
                                        angular.forEach(response.data, (d, i) => {
    ...
    .directive("materTracking", ["$http", "$timeout", ($http: ng.IHttpService, $timeout: ng.ITimeoutService): ng.IDirective=> {
        return {
            scope: {
            ...
            link: ($scope: any, element: JQuery, attrs: any) => {
                ...
                var load = () => {
                    $http.get(Utils.ServiceURI.Address() + "api/matertrack", {
                        ...
                        params: {
                            ...
                            angular.forEach(data.Reps, (rep: any, i) => {
                                if (!rep.IsFinish)
                ...
                $scope.$watch("workflowId+nodeId", (val) => {
    ...
    .directive("approvalRecords", ["$http", ($http: ng.IHttpService): ng.IDirective=> {
        return {
            restrict: "EA",
            scope: {
                workflowType: "@",
    ...
    .directive("actionLog", ["$http", ($http: ng.IHttpService): ng.IDirective=> {
        return {
            restrict: "EA",
            replace: true,
            scope: {
    ...
    .directive("contactInfo", ["$http", ($http: ng.IHttpService): ng.IDirective => {
        return {
            restrict: "EA",
            scope: {
                uscode: "=",
                ...
                $scope.$watch("uscode", (val) => {
                    if (!!val) {
                        ...
                        $http.get(url
                                ...
                                $scope.contactList = [];
                                angular.forEach(repData, (mgr: any, i) => {
    ...
    .directive("topNav", [
        "flowService",
        "taskWorkService",
        (flowService, taskWorkService): ng.IDirective=> {
            return {
                restrict: "E",
                scope: {
                    flowCode: "@",
    ...
    .factory("messager", [
        "$sce",
        "$modal",
        ($sce, $modal: ng.ui.bootstrap.IModalService) => {
            return {
                showMessage: (content: string, iconClass: string, modalSize: string) => {
                    ...
                    return $modal.open({
                        ...
                        controller: ["$scope", "$modalInstance", ($scope, $modalInstance: ng.ui.bootstrap.IModalServiceInstance) => {
                ...
                confirm: (content: string, iconClass: string) => {
                    return $modal.open({
                        backdrop: 'static',
                        size: "sm",
    ...
    ]).directive('reinvestmentBasicInfo', ["$http", "messager", "storeService", "dictionaryHandler", ($http, messager, storeService, dictionaryHandler): ng.IDirective=> {
        return {
            restrict: 'EA',
            replace: true,
            ...
            link: ($scope: any, element, attrs) => {
                ...
                $scope.$watch("code", (val) => {
                    if (val) {
                        $http.get(Utils.ServiceURI.Address() + "api/StoreSTLocation/" + $scope.code).then((response: any) => {
                        ...
                        dictionaryHandler.queryByParent({ parentCode: 'DesignType' }).$promise.then((dicts) => {
                            if (dicts != null) {
    ...
    }]).directive('reinvestmentInfo', ["$http", 'storeService', ($http, storeService): ng.IDirective=> {
        return {
            restrict: 'EA',
            replace: true,
    ...
    }]).directive('summaryReinvestmentCost', ["$http", "summaryReinvestmentCostService", ($http, summaryReinvestmentCostService): ng.IDirective=> {
        return {
            restrict: 'EA',
            replace: true,
    ...
    .directive('storeProfitabilityAndLeaseInfo', ["$http", "StoreProfitabilityAndLeaseInfoService", ($http, StoreProfitabilityAndLeaseInfoService): ng.IDirective=> {
        return {
            restrict: 'EA',
            replace: true,
    ...
    .directive('financialPreAnalysis', ["$http", "financialPreAnalysisService", '$filter', 'messager',
        ($http, financialPreAnalysisService, $filter, messager): ng.IDirective=> {
            return {
                restrict: 'EA',
                replace: true,
    ...
    .directive('writeOffAmount', (): ng.IDirective=> {
    return {
            restrict: 'EA',
            replace: true,
    ...
    .directive("dictSelector", ["dictionaryHandler", (dictionaryHandler): ng.IDirective=> {
        return {
            restrict: "E",
            replace: true,
            template: "\
            ...
            link: ($scope, element: JQuery, attrs) => {
                dictionaryHandler.queryByParent({ parentCode: $scope.parentCode }).$promise.then((dicts) => {
                    if ($scope.dictFilter) {
                        var dictsLocal = [];
                        angular.forEach(dicts, (d, i) => {
                    ...
                    if ($scope.defaultSelect
                        && $scope.dicts
                        && $scope.dicts.length == 1) {
                        $scope.selectedCode = $scope.dicts[0].Code;
                    }

                    $scope.loaded = true;
                });
                $scope.$watch("selectedCode+dicts", (val) => {
                    angular.forEach($scope.dicts, (d, i) => {
...
//                <div ng-if='loaded'>\
//                    <div ng-if='required'>\
//                        <select name='selectedCode' required chosen='{{chosenOptions}}' class='form-control' ng-model='$parent.$parent.selectedCode' ng-options='d.Code as d.NameZHCN for d in dicts'>\
//                            <option value = ''>请选择</option>\
//                        </select>\
//                        <err-msgs field='frmDict.selectedCode' submited='$root.$submited'></err-msgs>\
//                    </div>\
//                    <div ng-if='!required'>\
//                        <select name='selectedCode' chosen='{{chosenOptions}}' class='form-control' ng-model='$parent.$parent.selectedCode' ng-options='d.Code as d.NameZHCN for d in dicts'>\
//                            <option value = ''>请选择</option>\
//                        </select>\
//                    </div>\
//                </div>\
...
//    };
//}])
    .directive("dictMutiSelector", ["dictionaryHandler", (dictionaryHandler): ng.IDirective=> {
        return {
            restrict: "E",
            replace: true,
            template: "\
    ...
    }]).directive('reinvestmentCost', ["$window", 'messager', ($window, messager): ng.IDirective=> {
        return {
            restrict: "EA",
            replace: true,
            scope: {
            ...
            link: ($scope: any, ele: JQuery, attrs: any) => {
                ...
                $scope.saveBasicInfo = () => {
                    if ($scope.source
                        && ($scope.source.NormType || $scope.hideNormType)) {
    ...
    }]).directive("reinvstcostAndWriteoff", ['$filter', ($filter): ng.IDirective=> {
        return {
            restrict: "EA",
            replace: true,
            scope: {
            ...
            link: ($scope: any, ele: JQuery, attrs: any) => {
                $scope.$watch('inventCostSource.RECostNorm+inventCostSource.LHINorm+inventCostSource.ESSDNorm', function (newVal, oldVal) {
                    ...
                    if ($scope.reinvenstmentType == 2
                        && $scope.inventCostSource
                        && newVal != oldVal) {
        ...
        "storeService",
        '$filter',
        (siteInfoService, storeService, $filter): ng.IDirective => {
            return {
                restrict: "EA",
                ...
                link: ($scope: any, element: JQuery, attrs: any) => {
                    ...
                    var loadDropdown = () => {
                        siteInfoService.getDropdownDatas().$promise.then((data) => {
                            if (data != null) {
                    ...
                    $scope.$watch("usCode", (val) => {
                        if (val != null && val != "") {
                            $scope.dataLoaded = false;
                            siteInfoService.getSiteInfo({ usCode: val, identifier: $scope.identifier, projectId: $scope.projectId, flowCode: $scope.flowCode }).$promise.then((data: any) => {
                                $scope.source = data;
                                $scope.source.PlayPlace = data.PlayPlace;
                                $scope.source.PartyRoom = data.PartyRoom;
                                loadDropdown();
                            });
                            storeService.getStoreBasic({ usCode: val }).$promise.then((data) => {
                                if (data != null) {
                                    $scope.store = data;
                                }
                            });
                    ...
                    $scope.getSeatingRatio = function () {
                        if ($scope.source
                            && $scope.source.TotalArea
                            && $scope.source.TotalSeatsNo) {
        ...
        "$http", ($http: ng.IHttpService): ng.IDirective => {
            return {
                restrict: "EA",
                scope: {
                    pageUrl: "@",//历史记录页面的地址
                    ...
                    tableName: "@",//业务表名
                    title: "@",//标题名称
                    hasTemplate: "=?",//表单数据是否与模板附件关联（ture/false)
                    titleName: "@"
                },
                replace: true,
                templateUrl: Utils.ServiceURI.AppUri + "Module/ProjectHistory",
                link: ($scope: any, ele: JQuery, attrs: any, ngModelCtrls: any) => {
                    ...
                    $scope.$watch("projectId", (val) => {
                        ...
                        if (!!val) {
                            ...
                            $http.get(Utils.ServiceURI.Address() + "api/project/GetHistory/" + val + "/" + $scope.tableName + "/" + $scope.hasTemplate).success((historyData) => {
                                if (historyData != "") {
                                    ...
                                    angular.forEach($scope.historyList, (history, i) => {
    ...
    .directive("notificationMessage", ["$http",
        "$window",
        "$selectUser", "messager", ($http: ng.IHttpService, $window: ng.IWindowService, $selectUser, messager): ng.IDirective => {
        return {
                restrict: "EA",
                scope: {
                    projectId: "=",
                ...
                link: ($scope: any, ele: JQuery, attrs: any, ngModelCtrls: any) => {
                    ...
                    $scope.selectEmployee = () => {
                        var users = $.map($scope.notifyUsersInfo.NoticeUsers, (u, i) => {
                            return {
                                Code: u.UserAccount,
                                NameZHCN: u.UserNameZHCN,
                                NameENUS: u.UsernameENUS
                        ...
                        $selectUser.open({
                            ...
                            selectedUsers: angular.copy(users || []),
                            OnUserSelected: (selectedUsers: any[]) => {
                                $scope.noticeUsersName = $.map(selectedUsers, (u, i) => {
                                    return u.NameENUS;
                                }).join(",");
                                $scope.notifyUsersInfo.NoticeUsers = $.map(selectedUsers, (u, i) => {
                                    return {
                                        UserAccount: u.Code,
                                        UserNameZHCN: u.NameZHCN,
                                        UsernameENUS: u.NameENUS
                    ...
                    $scope.sendMsg = function () {
                        ...
                        if ($scope.nodeId == null
                            || $scope.nodeId == ""
                            || $scope.nodeId == "00000000-0000-0000-0000-000000000000") {
                    ...
                    $scope.gotoAllMsg = function () {
                        $window.location.href = Utils.ServiceURI.WebAddress()
                        + "Home/Main#/CommentsList?projectId=" + $scope.projectId
                        + "&flowCode=" + $scope.flowCode
                        + "&subFlowCode=" + $scope.subFlowCode;
                    };
                    ...
                    var loadMessageList = function () {
                        $http.get(Utils.ServiceURI.Address()
                            + "api/NotificationMessage/GetMessageList/"
                            + window["currentUser"].Code + "/"
                            + $scope.projectId + "/"
                            + $scope.subFlowCode).success(function (MessageList) {
    ...
    .directive("fileDownloader", [
        "$timeout",
        "messager",
        ($timeout, messager): ng.IDirective=> {
            return {
                restrict: "A",
                link: ($scope: any, element: JQuery, attrs: any) => {
                    var timer;
    ...
    .directive("packDownloader", ["$http", "messager", ($http: ng.IHttpService, messager): ng.IDirective=> {
        return {
            restrict: "A",
            scope: {
                refTableName: "@",
            ...
            replace: false,
            link: ($scope: any, ele: JQuery, attrs) => {
    ...
    .directive("reopenMemo", ["$http", "messager", ($http: ng.IHttpService, messager): ng.IDirective => {
        return {
            restrict: "EA",
            scope: {
                projectId: "@",
            ...
            link: ($scope: any, element: JQuery, attrs: any) => {
                ...
                $scope.DeleteImg = function (typeCode) {
                    ...
                    $http.post(Utils.ServiceURI.Address() + "api/ReopenMemo/DeleteImg/" + typeCode + "/" + $scope.projectId, {
    ...
    .directive("gbMemo", ["$http", "messager", ($http: ng.IHttpService, messager): ng.IDirective => {
        return {
            restrict: "EA",
            scope: {
                projectId: "@",
    ...
    .directive("tempClosureMemo", ["$http", "messager", ($http: ng.IHttpService, messager): ng.IDirective => {
        return {
            restrict: "EA",
            scope: {
                projectId: "@",
    ...
    .directive("leaseChange", ["$http", "messager", ($http: ng.IHttpService, messager): ng.IDirective => {
        return {
            restrict: "EA",
            scope: {
                projectId: "@",
    ...
    .factory('projectStatusChangeService', [
        ...
        $modal => {
            return {
                open: (projectId, flowCode, status, usCode) => {
                    return $modal.open({
                        backdrop: 'static',
                        ...
                        controller: [
                            "$scope", "$modalInstance", "messager", 'projectService', 'storeService', 'model', ($scope, $modalInstance, messager, projectService, storeService, model) => {
                                $scope.model = model;
                                ...
                                projectService.getProjectStatusChangeLog({ projectId: projectId }).$promise.then(result => {
                                    $scope.changelogs = result.changeLog;
                                });

                                $scope.loadProjectStatusOpts = (() => {
                                    switch (status) {
                                        case 0:
                                        case 7:
                                        case 3:
    ...
    .factory("redirectService", [
        ...
        ($location, $window) => {
            return {
                flowRedirect: (flowCode, projectId) => {
    ...
    .directive('approvedPackage', [
        ...
        "$http",
        "messager",
        ($modal, $http, messager): ng.IDirective=> {
            return {
                restrict: "E",
                scope: {
                    entityId: "=?",

reimageApp.controller("reopenMemoController", [
    '$scope',
    '$routeParams',
    "$location",
    "$selectUser",
    ...
    function ($scope, $routeParams, $location, $selectUser, $window, taskWorkService, reopenMemoService, redirectService, messager) {
        ...
        $scope.notify = function (frm) {
            ...
            $selectUser.open({
                ...
                OnUserSelected: function (users) {
                    $scope.acting = true;
                    messager.confirm("确定要进行发送吗？", "fa-warning c_orange").then(function (result) {
                        if (result) {
                            reopenMemoService.sendReopenMemo({
                                ...
                                messager.showMessage("[[[发送成功]]]", "fa-check c_green").then(function () {
                                    ...
                                    redirectService.flowRedirect("Reimage_ReopenMemo", $scope.projectId);
                                });
                            }, function (response) {
        ...
        taskWorkService.ifUndo("Reimage_ReopenMemo", $scope.projectId).then(function (result) {

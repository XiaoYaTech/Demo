reimageApp.controller("consInvtCheckingController", ["$http",
    "$scope",
    "$window",
    "$modal",
    "$location",
    ...
    'redirectService',
    "messager",
    function ($http, $scope, $window, $modal, $location, $routeParams, reimageService, approveDialogService, redirectService, messager) {
        ...
        $scope.subFlowCode = "Reimage_ConsInvtChecking";
        ...
        var loadConsInfo = function () {
            return reimageService.getConsInfo({ projectId: $scope.projectId }).$promise.then(function (data) {
        ...
        var loadCheckingInfo = function () {
            return reimageService.getConsInvtChecking({ projectId: $scope.projectId, entityId: $scope.entityId }).$promise.then(function (data) {
                if (data != null && data.Id != "00000000-0000-0000-0000-000000000000") {
                    $scope.ConsInvtChecking = data;
                    loadInvestCost();
                    loadWriteOff();
                }
                if (data != null && data.IsShowSave && $scope.pageType == 'View') {
        ...
        var loadInvestCost = function () {
            ...
            { return; }
            reimageService.getInvestCost({ refTableId: $scope.ConsInvtChecking.Id }).$promise.then(function (cost) {
                $scope.ReinCost = cost;
            }, function (attError) {
        ...
        var loadWriteOff = function () {
            if ($scope.ConsInvtChecking.Id == null)
            { return; }
            reimageService.getWriteOff({ refTableId: $scope.ConsInvtChecking.Id }).$promise.then(function (writeoff) {
        ...
        var save = function (action) {
            ...
            if (action == "save") {
                reimageService.saveConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (data) {
                ...
                messager.confirm("确定要进行提交吗？", "fa-warning c_orange").then(function (result) {
                    if (result) {
                        reimageService.submitConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (data) {
                            messager.showMessage("[[[提交成功]]]", "fa-check c_green").then(function () {
                                redirectService.flowRedirect($scope.ConsInvtChecking.WorkflowCode, $scope.projectId);
                            });
                        }, function (error) {
        ...
        var checkValidate = function () {
            ...
            if ($scope.WriteOff.REActual && $scope.WriteOff.REWriteOff && !$scope.WriteOff.ExpFAActVsReCost) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("缺少描述信息，请在模板中填写Description for RE Cost Variance>+-5%后再重新上传");
                }
            }
            if ($scope.WriteOff.LHIActual && $scope.WriteOff.LHIWriteOff && !$scope.WriteOff.ExpFAActVsLHI) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("缺少描述信息，请在模板中填写Description for LHI Variance>+-5%后再重新上传");
                }
            }
            ...
            if (tESSD && tESSDAc && !$scope.WriteOff.ExpFAActVsESSD) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("缺少描述信息，请在模板中填写Description for ESSD Variance>+-5%后再重新上传");
                }
            }
            ...
            if ($scope.WriteOff.TotalActual && $scope.WriteOff.TotalWriteOff && !$scope.WriteOff.ExpFAActVsTotal) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("缺少描述信息，请在模板中填写Description for Total Variance>+-5%后再重新上传");
                }
            }
            ...
            if ($scope.ReinCost.RECostFAAct && $scope.ReinCost.RECostBudget && !$scope.ReinCost.ExpFAActVsReCost) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("请填写Explanation_FA Actual vs. Budget (RE Cost)");
                }
            }
            if ($scope.ReinCost.LHIFAAct && $scope.ReinCost.LHIBudget && !$scope.ReinCost.ExpFAActVsLHI) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("请填写Explanation_FA Actual vs. Budget (LHI)");
                }
            }
            if ($scope.ReinCost.ESSDFAAct && $scope.ReinCost.ESSDBudget && !$scope.ReinCost.ExpFAActVsESSD) {
                ...
                if (Math.abs(v) > 5) {
                    ...
                    //return false;
                    errors.push("请填写Explanation_FA Actual vs. Budget (ESSD)");
                }
            }
            if ($scope.ReinCost.TotalReinvestmentFAAct && $scope.ReinCost.TotalReinvestmentBudget && !$scope.ReinCost.ExpFAActVsTotal) {
                ...
                if (Math.abs(v) > 5) {
                    //messager.showMessage("请填写Explanation_FA Actual vs Budget (Total)", "fa-warning c_orange");
                    //return false;
                    errors.push("请填写Explanation_FA Actual vs Budget (Total)");
                }
            }
            if (errors.length > 0) {
        ...
        var resubmit = function () {
            ...
            messager.confirm("确定要进行提交吗？", "fa-warning c_orange").then(function (result) {
                if (result) {
                    reimageService.resubmitConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
                        messager.showMessage("[[[提交成功]]]", "fa-check c_green").then(function () {
                            redirectService.flowRedirect($scope.ConsInvtChecking.WorkflowCode, $scope.projectId);
                        });
                    }, function (error) {
        ...
        $scope.submit = function (frm) {
            ...
            //}
            if (checkValidate()) {
                approveDialogService.open($scope.projectId, $scope.subFlowCode, $scope.ConsInvtChecking.ApprovalType, "", $scope.ConsInvtChecking).then(function (approverInfo) {
        ...
        $scope.approve = function () {
            ...
            messager.confirm("确定要进行审批吗？", "fa-warning c_orange").then(function (result) {
                if (result) {
                    reimageService.approveConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
              ...
              .then(function (result) {
                  if (result) {
                      $modal.open({
                          ...
                          reimageService.recallConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
        ...
        $scope.edit = function () {
            messager.confirm("[[[Construction Investment Checking 审批流程已经完成，现在编辑会导致流程重新提交，是否确认？]]]", "fa-warning c_orange")
                .then(function (result) {
                    if (result) {
                        reimageService.editConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (response) {
        ...
        $scope.returnToOriginator = function () {
            ...
            messager.confirm("[[[确定要进行退回吗？]]]", "fa-warning c_orange").then(function (result) {
                if (result) {
                    reimageService.returnConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
        ...
        //reimageService.getMajorInfo({ projectId: $scope.projectId }).$promise.then(function (data) {

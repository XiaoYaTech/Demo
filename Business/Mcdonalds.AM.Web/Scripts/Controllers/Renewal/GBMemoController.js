!function() {
    ...
    RenewalApp.controller("GBMemoController", [
        '$scope',
        '$routeParams',
        "$modal",
        "$location",
        ...
        "redirectService",
        "messager",
        function ($scope, $routeParams, $modal, $location, $selectUser, $window, taskWorkService, approveDialogService, renewalService,redirectService, messager) {
            ...
            $scope.subFlowCode = "Renewal_GBMemo";
            $scope.checkPointRefresh = true;
            $scope.entity = {};

            var loadGBMemoInfo = function() {
                return renewalService.getGBMemoInfo({ projectId: $scope.projectId, entityId: $scope.entityId }).$promise.then(function (data) {
                        if (data != null) {
                            $scope.entity = data;
                        }
                    },
            ...
            var save = function(action) {
                ...
                if (action == "save") {
                    ...
                    renewalService.saveGBMemo($scope.entity).$promise.then(function (data) {
                        loadGBMemoInfo();
                    ...
                    $scope.IsClickSubmit = true;
                    $scope.IsClickSave = true;
                    renewalService.submitGBMemo($scope.entity).$promise.then(function (data) {
            ...
            var resubmit = function() {
                ...
                $scope.IsClickSubmit = true;
                $scope.IsClickSave = true;
                renewalService.resubmitGBMemo($scope.entity).$promise.then(function () {
                    messager.showMessage("[[[提交成功]]]", "fa-check c_green").then(function() {
                        redirectService.flowRedirect($scope.entity.WorkflowCode, $scope.projectId);
                    });
                }, function(error) {
            ...
            $scope.approve = function () {
                ...
                $scope.entity.LastUpdateUserNameZHCN = window.currentUser.NameZHCN;
                $scope.entity.LastUpdateUserNameENUS = window.currentUser.NameENUS;
                renewalService.approveGBMemo($scope.entity).$promise.then(function () {
                    messager.showMessage("[[[提交成功]]]", "fa-check c_green").then(function () {
                        redirectService.flowRedirect($scope.entity.WorkflowCode, $scope.projectId);
                    });
                }, function (error) {
                    ...
                    .then(function(result) {
                        if (result) {
                            $modal.open({
                                ...
                                controller: [
                                    "$scope", "$modalInstance", "entity", function($modalScope, $modalInstance, entity) {
                                ...
                                renewalService.recallGBMemo($scope.entity).$promise.then(function () {
            ...
            $scope.edit = function() {
                messager.confirm("[[[GBMemo 审批流程已经完成，现在编辑会导致流程重新提交，是否确认]]]？", "fa-warning c_orange")
                    .then(function(result) {
                        if (result) {
                            renewalService.editGBMemo($scope.entity).$promise.then(function (response) {
            ...
            $scope.returnToOriginator = function() {
                ...
                messager.confirm("[[[确认需要Return吗？]]]", "fa-warning c_orange").then(function(result) {
                    if (result) {
                        ...
                        $scope.entity.LastUpdateUserNameENUS = window.currentUser.NameENUS;
                        $scope.entity.SerialNumber = $routeParams.SN;
                        renewalService.returnGBMemo($scope.entity).$promise.then(function () {
            ...
            taskWorkService.ifUndo("Renewal_GBMemo", $scope.projectId).then(function (result) {

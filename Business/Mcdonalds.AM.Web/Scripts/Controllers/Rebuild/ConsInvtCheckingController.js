rebuildApp.controller("consInvtCheckingController", ["$http",
    "$scope",
    "$window",
    "$modal",
    "$location",
    ...
    function ($http, $scope, $window, $modal, $location, $routeParams, rebuildService, approveDialogService,redirectService, messager) {
        ...
        $scope.subFlowCode = "Rebuild_ConsInvtChecking";
        ...
        var loadConsInfo = function () {
            return rebuildService.getConsInfo({ projectId: $scope.projectId }).$promise.then(function (data) {
        ...
        var loadCheckingInfo = function () {
            return rebuildService.getConsInvtChecking({ projectId: $scope.projectId, entityId: $scope.entityId }).$promise.then(function (data) {
        ...
        var loadInvestCost = function () {
            ...
            { return; }
            rebuildService.getInvestCost({ refTableId: $scope.ConsInvtChecking.Id }).$promise.then(function (cost) {
                $scope.ReinCost = cost;
            }, function (attError) {
        ...
        var loadWriteOff = function () {
            if ($scope.ConsInvtChecking.Id == null)
            { return; }
            rebuildService.getWriteOff({ refTableId: $scope.ConsInvtChecking.Id }).$promise.then(function (writeoff) {
        ...
        var save = function (action) {
            ...
            if (action == "save") {
                rebuildService.saveConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (data) {
                    messager.showMessage("[[[保存成功]]]", "fa-check c_green").then(function () {
                    });
                }, function (error) {
                    $scope.IsClickSubmit = false;
                    messager.showMessage("[[[提交失败]]]" + error.statusText, "fa-warning c_orange");
                });
            }
            else if (action == "submit") {
                rebuildService.submitConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (data) {
        ...
        var resubmit = function () {
            ...
            rebuildService.resubmitConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
                messager.showMessage("[[[提交成功]]]", "fa-check c_green").then(function () {
                    redirectService.flowRedirect($scope.ConsInvtChecking.WorkflowCode, $scope.projectId);
                });
            }, function (error) {
        ...
        $scope.approve = function () {
            ...
            $scope.ConsInvtChecking.SerialNumber = $routeParams.SN;
            rebuildService.approveConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
              ...
              .then(function (result) {
                  if (result) {
                      $modal.open({
                          ...
                          rebuildService.recallConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
                              messager.showMessage("[[[撤回成功]]]", "fa-check c_green").then(function () {
                                  redirectService.flowRedirect($scope.ConsInvtChecking.WorkflowCode, $scope.projectId);
                              });
                          }, function (error) {
                              messager.showMessage("[[[撤回失败]]]" + error.statusText, "fa-warning c_orange");
                          });
                      });
                  }
              });
        ...
        $scope.edit = function () {
            messager.confirm("[[[Construction Investment Checking 审批流程已经完成，现在编辑会导致流程重新提交，是否确认？]]]", "fa-warning c_orange")
                .then(function (result) {
                    if (result) {
                        rebuildService.editConsInvtChecking($scope.ConsInvtChecking).$promise.then(function (response) {
        ...
        $scope.returnToOriginator = function () {
            ...
            messager.confirm("[[[确认需要Return吗？]]]", "fa-warning c_orange").then(function (result) {
                if (result) {
                    ...
                    $scope.ConsInvtChecking.SerialNumber = $routeParams.SN;
                    rebuildService.returnConsInvtChecking($scope.ConsInvtChecking).$promise.then(function () {
                        messager.showMessage("[[[退回成功]]]", "fa-check c_green").then(function () {
                            redirectService.flowRedirect($scope.ConsInvtChecking.WorkflowCode, $scope.projectId);
                        });
                    }, function (error) {
                        messager.showMessage("[[[退回失败]]]" + error.statusText, "fa-warning c_orange");
